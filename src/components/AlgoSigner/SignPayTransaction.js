...
const SignPayTransaction = ({
  ...
  const signPayTransaction = async () => {
    ...
    try {
      ...
      let Transactionid = await client.sendRawTransaction(binarySignedTx).do();
    ...
    if (amount.current < 2) {
      try {
        ...
        // Send the transaction through the SDK client
        let Transactionid = await client
    ...
    if (amount.current >= 2) {
      try {
        const txn =
          await new algosdk.makePaymentTxnWithSuggestedParamsFromObject({
            from: userAccount.current[0].address, //Crowdfund
            to: "VIOIRTNMQWJKYTEMGYRBKV3LUJ7AMRWMJCJ45XEMHCJHNMUHI6MHB4UKZI", //receipient.current,
            // amount:parseInt(amount.current),
            amount: 0.1 * algosToMicroalgos(parseInt(amount.current)), //match donation by 10% or give back 10% to sender
            // note: "document.getElementById('note').value",
            suggestedParams: { ...suggestedParams },
  ...
  return (
    <div>
      <div>
        ...
        <br />
        <FormStyle
          onChange={(e) => (receipient.current = e.target.value)}
          placeholder="Receiver address"
        />
        <br />
       
        <TransactionButton backgroundColor onClick={signPayTransaction}>
